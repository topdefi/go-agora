...
`
	...
	{
		agora := runGeth(t, "account", "list", "--datadir", datadir)
		agora.Expect(want)
		agora.ExpectExit()
	}
	{
		agora := runGeth(t, "--datadir", datadir, "account", "list")
		agora.Expect(want)
		agora.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	agora := runGeth(t, "account", "new", "--lightkdf")
	defer agora.ExpectExit()
	agora.Expect(`
...
func TestAccountHelp(t *testing.T) {
	agora := runGeth(t, "account", "-h")
	agora.WaitExit()
	if have, want := agora.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	agora = runGeth(t, "account", "import", "-h")
	agora.WaitExit()
	if have, want := agora.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	agora := runGeth(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer agora.ExpectExit()
	agora.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	agora := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	agora := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer agora.ExpectExit()
	agora.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(agora.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	agora := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(agora.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	agora.SetTemplateFunc("keypath", func(file string) string {
