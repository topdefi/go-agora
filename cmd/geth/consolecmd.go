...
See https://agora.ethereum.org/docs/interacting-with-agora/javascript-console.`,
	}
...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://agora.ethereum.org/docs/interacting-with-agora/javascript-console.
This command allows to open a console on a running agora node.`,
	}
...
JavaScript API. See https://agora.ethereum.org/docs/interacting-with-agora/javascript-console`,
	}
)

// localConsole starts a new agora node, attaching a JavaScript console to it at the
// same time.
...
// remoteConsole will connect to a remote agora instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Unable to attach to remote agora: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.String(utils.JSpathFlag.Name),
...
// ephemeralConsole starts a new agora node, attaches an ephemeral JavaScript
