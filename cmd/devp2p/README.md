...
1. initialize the agora node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run agora with the following flags:
```
agora --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the agora node:
 ```
...
To run the eth66 protocol test suite, initialize a agora node as described above and run the following command,
replacing `<enode>` with the enode of the agora node:
...
[dns-tutorial]: https://agora.ethereum.org/docs/developers/agora-developer/dns-discovery-setup
