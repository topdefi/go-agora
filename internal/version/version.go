...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "agora".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("agora %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("agora %s", params.VersionWithMeta)
	}
