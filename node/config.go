...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of agora is "agora". If no
...
func (c *Config) NodeName() string {
	...
	if name == "agora" || name == "agora-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "agora" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by agora 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGethResourceWarning {
				...
				log.Warn("Using deprecated resource file, please move this file to the 'agora' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
